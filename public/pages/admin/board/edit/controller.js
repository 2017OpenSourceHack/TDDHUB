app.controller('admin.board.edit', function($scope, $http, $state, $stateParams){
  ...
  $scope.item = {
    "type" : "기술지원",
      "title" :"서버 점검 안내",
      "detail" :"11월 25일  00시~ 04시에 서버 점검이 이루어질 예정입니다. 이 기간동안 카페갈래 서비스는 이용하실 수 없습니다. 감사합니다.",
      "date" :" 2016-11-22",
      "filePath" : "C:\test.jpg"
  }
  ...
  $scope.list = function () {
    // console.log($scope.selected.keyword)
    $http.get('/notices/'+$stateParams.id).then(function (res)  {
      console.log(res)
  ...
  $scope.save = function (item) {
    ...
    console.log(item)
    // $http.delete('/cafeinfos/' + selected.item.sid).then(function () {
    //   $scope.list()
    // })

    delete item._id

    $http.put('/notices/' + item.sid, item).then(function (res) {
      $state.go('admin.board.list')
    })
